...
#if defined(JET_USE_JETMALLOC) && defined(_MSC_VER)
#pragma comment(lib, "..\\..\\bin\\jet_malloc.lib")
#include "../../jet.malloc/include/jet_malloc.h"
#endif

#if defined (JET_USE_JETMALLOC) && defined(__linux__)
#include "../../jet.malloc/include/jet_malloc.h"
#endif
...
#ifdef JET_USE_JETMALLOC
	return jm_malloc(bytes);
#else
	return malloc(bytes);
#endif
}

static JS_INLINE void* js_aligned_malloc(size_t bytes, size_t align) {
#ifdef JET_USE_JETMALLOC
	return jm_aligned_malloc(bytes, align);
#else
	return malloc(bytes);
#endif
}
...
static JS_INLINE void js_aligned_free(void* p) {
#ifdef JET_USE_JETMALLOC
	jm_aligned_free(p);
#else
	free(p);
